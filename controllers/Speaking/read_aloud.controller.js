...
async function GetReadAloud(req, res) {
  try {
...
async function CheckStudentScore(req, res) {
  try {
...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    cb(null, audio_file.fieldname + "-" + Date.now());
  },
});


const upload = multer({ storage }).single("audio_file");
// console.log(upload);
async function SubmitAnswer(req, res) {
  try {
    ...
    upload(req, res, async err => {
      if (err) {
        return res.status(400).json({ error: "Failed to upload audio file", details: err });
      }
      const audioFilePath = req.file ? req.file.path.replace("public", "") : null; // Store relative path in DB
